...
async function handleSidekick(request, env, ctx, corsOrigin) {
  return runStandardHandler({
    ...
    buildSystemPrompt: () => (
      ...
      'OFF-LIST EXAMPLES. When you give an example to explain a concept, NEVER use any of the multiple-choice options as your example — pick something different. If the choices are [Canada, France, Brazil], use "Japan" or "Egypt" as your example country. If the choices are [cat, dog, fish], use "rabbit" as your example animal. If the choices are numbers like [12, 15, 18], use a totally different number (e.g., 7) for any worked example. This rule is critical: a downstream safety filter rejects any reply that contains a choice text, so referencing one will look like an offline error to the kid even though you didn\'t mean to leak.\n\n' +
